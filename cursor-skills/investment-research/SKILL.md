...
1. 读取 `reports/重点标的看板.md` 和 `reports/标的跟踪表.md`，用公司全称和股票代码识别现有记录。
2. 如果标的已在重点标的看板：更新该行的最新报告、研究阶段、本周关注和下次动作；不在标的跟踪表重复建行。
3. 如果标的不在重点标的看板：在 `reports/标的跟踪表.md` 主表中新增或更新一条建议事件，至少填写：
   - `ID`：`{YYYYMMDD}-{代码或规范简称}`；
