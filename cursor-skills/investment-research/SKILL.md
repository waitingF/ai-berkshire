...
description: "AI Berkshire skill: 投资研究：巴菲特-芒格-段永平-李录 四大师综合分析框架. Use when the user asks for /investment-research or investment research workflows from this project. Source: skills/investment-research.md."
---

## Cursor adapter note

This skill is generated from `skills/investment-research.md` so Claude Code, Codex, and Cursor users share one canonical workflow.
...
3. 每个模块末尾必须有对应大师的"追问"
4. 最终将完整报告写入 `reports/{公司名}/{公司名}-research-{YYYYMMDD}.md`（目录不存在则创建；文件名保持该格式）
...
报告通过数据抽检后、向用户回复前，对**每个已完成研究的标的**执行以下收尾：

1. 读取 `reports/重点标的看板.md` 和 `reports/买卖建议跟踪表.md`，用公司全称和股票代码识别现有记录。
2. 如果标的已在重点标的看板：更新该行的最新报告、研究阶段、本周关注和下次动作；不在买卖建议跟踪表重复建行。
3. 如果标的不在重点标的看板：在 `reports/买卖建议跟踪表.md` 主表中新增或更新一条建议事件，至少填写：
   - `ID`：`{YYYYMMDD}-{代码或规范简称}`；
   - 标的全称与股票代码、建议日、研究报告相对链接；
   - 动作（买入/分批/观望/回避）、锚定价、触发条件、复检日；
   - 初始状态为“待触发”，结果和复盘可填“—”，升级填“无”。
4. 同一标的、同一来源报告**禁止重复新增**；重跑技能时更新原事件行。
5. 更新跟踪表的“数据截止日”“上次更新”“状态与命中率摘要”和总事件数，所有计数必须从主表重新统计。
6. 验证新增的本地报告链接存在，并在最终回复中说明该标的是新增、更新，还是因已属重点标的而分流到重点标的看板。

自动跟踪是完成条件，不是可选建议。若报告已经生成但台账无法安全更新，必须明确报告“**跟踪表更新失败**”及原因，不得声称整个流程已完成。
