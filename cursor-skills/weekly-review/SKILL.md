---
name: weekly-review
description: "AI Berkshire skill: 周检：重点优先的研究待办分诊. Use when the user asks for /weekly-review or investment research workflows from this project. Source: skills/weekly-review.md."
---

## Cursor adapter note

This skill is generated from `skills/weekly-review.md` so Claude Code, Codex, and Cursor users share one canonical workflow.
...
价位带、现价、判定、事件等**机器事实不逐条复述**每日触发扫描
（`reports/trigger-scan/trigger-scan-latest.md`），一律引用当日扫描；周检只写人工核验增量与决策。
Pages 首页「监控与周检」入口由构建脚本组合扫描 latest 与周检 latest 生成，周检模板与之保持一致。
...
1. “本周待办队列”“重点标的状态”“非重点台账触发项（决策增量）”三个表格，以及“本周不处理项”中的每个标的名称都使用本地报告链接；不要只在文末集中列出来源。
...
## 三、非重点台账触发项（决策增量，不复述扫描）

完整价位带、现价、判定与事件以当日
[`trigger-scan-latest.md`](../trigger-scan/trigger-scan-latest.md) 为准，**本表只列需要人工决策或周检新增了事实**的条目；扫描已覆盖的价位、现价、判定、动作、备注不再复述。

| 标的（链接主报告） | 台账状态 | 周检新增事实/决策点 | 建议下一步 |
|---|---|---|---|
...
- **扫描事实不重述**：价位带、现价、事件以当日触发扫描为准，周检只写人工核验增量与决策，避免两页重复维护。
