...
本 Skill 是**只读、联网、重点优先的分诊台**：输出按优先级排序的待办清单，不写入或修改任何报告、看板、台账、thesis 或组合文件。
...
4. 从买卖建议跟踪表提取：标的、代码、动作、锚定价、触发条件、复检日、状态、升级字段和来源报告。
...
2. 有明确买入、分批、观望或回避建议、却未被重点看板或买卖建议台账覆盖的报告；
...
输出一份只在当前对话呈现的 Markdown 周检报告，固定使用以下结构：
...
- **只读但可行动**：输出能直接指导下一项 workflow，却不替用户作投资决定。
