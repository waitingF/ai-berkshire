...
description: "AI Berkshire skill: 巴菲特价值投资买入前 Checklist. Use when the user asks for /investment-checklist or investment research workflows from this project. Source: skills/investment-checklist.md."
---

## Cursor adapter note

This skill is generated from `skills/investment-checklist.md` so Claude Code, Codex, and Cursor users share one canonical workflow.
...
将完整报告写入：
- **单公司**：`reports/{公司名}/{公司名}-checklist-{YYYYMMDD}.md`（目录不存在则创建）
- **多公司对比**：`reports/多公司对比-checklist-{YYYYMMDD}.md`（放 reports/ 根目录）

### 第八步：自动加入重点标的（必须执行）

Checklist 报告写入后、向用户回复前，将**每个执行 Checklist 的标的**加入或更新到 `reports/重点标的看板.md`，**无论 Checklist 结论**是通过、未通过、灰色地带还是 N/A：

1. 用公司全称和股票代码查重；已有行时更新原行，**禁止重复新增**。
2. 新行至少填写：标的、研究阶段 `checklist`、最新 Checklist 报告、Thesis 健康度 `⚪无thesis`、结论/争议摘要、本周关注、下次动作和报告链接。
3. 多公司 Checklist 必须逐一写入主表，不得只登记总览报告。
4. 若该标的已在 `reports/买卖建议跟踪表.md`，将其视为升格：**移除该标的的旧事件行**，并从主表重新计算跟踪表的状态汇总和总事件数，避免两张表重复维护。
5. 更新重点标的看板的“数据截止日”“上次更新”、非重点标的摘要计数和待办汇总；本地报告链接必须存在。
6. 最终回复说明每个标的是新增重点标的还是更新既有重点标的。

重点标的入表是完成条件。若报告已经生成但看板无法安全更新，必须明确报告“**重点标的更新失败**”及原因，不得声称整个流程已完成。
