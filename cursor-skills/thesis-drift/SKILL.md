---
name: thesis-drift
description: "AI Berkshire skill: 投资论文漂移检测：分清事实变化与措辞变化. Use when the user asks for /thesis-drift or investment research workflows from this project. Source: skills/thesis-drift.md."
---

## Cursor adapter note

This skill is generated from `skills/thesis-drift.md` so Claude Code, Codex, and Cursor users share one canonical workflow.
...
- `公司名` — 自动查找 `reports/{公司名}/{公司名}-thesis.md` 及同目录历史快照；如果没有基线则转入缺失基线处理
...
- 如果只提供公司名 → 查找 `reports/{公司名}/{公司名}-thesis.md` 及历史快照，进入**自动快照对比**模式
...
4. 如果当前报告已足够完整，可建议将它保存为 `reports/{公司名}/{公司名}-thesis.md` 作为未来漂移检测基线
